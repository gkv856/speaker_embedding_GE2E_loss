...
class CreateSpectrogram:
    ...
    def save_spectrogram_tisv(self):
        ...
        for i, folder in enumerate(lst_all_speaker_folders):
            ...
            self.__save_mel_spects(utter_train, folder, training=True)

            # saving test data
            self.__save_mel_spects(utter_test, folder, training=False)

        print("Spectrograms saved!!")

    def __save_mel_spects(self, mel_spects, folder, training=True):
        purpose = "train"
        m_name = "GE2E"
        if training:
